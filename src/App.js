...
const CONTRACT_ADDRESS = '0x4dc826be6fbeccaec2f767e0d8af3c55a22973d2';
...
const ASSETS = [
  { name: 'WETH', symbol: 'WETH', decimals: 18 },
  { name: 'cbBTC', symbol: 'cbBTC', decimals: 8 },
  { name: 'VIRTUAL', symbol: 'VIRTUAL', decimals: 18 },
  { name: 'AERO', symbol: 'AERO', decimals: 18 },
  { name: 'WELL', symbol: 'WELL', decimals: 18 }
];
...
function App() {
  ...
  const loadBalances = async () => {
    ...
    try {
      ...
      setBalances([
        Number(ethers.formatUnits(result[0], 18)),
        Number(ethers.formatUnits(result[1], 8)),
        Number(ethers.formatUnits(result[2], 18)),
        Number(ethers.formatUnits(result[3], 18)),
        Number(ethers.formatUnits(result[4], 18))
      ]);
    } catch (err) {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      try {
        ...
        setError('⚠️ Transaction will likely fail. Reasons: 1) Insufficient liquidity in Aerodrome pools 2) You need more ETH for gas 3) Try a smaller amount');
        setLoading(false);
        return;
      }
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (err.code === 4001 || err.code === 'ACTION_REJECTED') {
        ...
        setError('⚠️ Withdrawal failed on-chain. Likely reasons: 1) Amounts too small to swap (need minimum liquidity) 2) DEX pools lack liquidity. Consider depositing more first.');
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-3 sm:p-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-8 gap-3 sm:gap-0">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-blue-400">Base5</h1>
            <p className="text-gray-400 text-xs sm:text-sm mt-1">Auto-diversify into top 5 Base tokens</p>
            ...
            {activeTab === 'deposit' ? (
              <div className="space-y-4 sm:space-y-6">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Minimum: 10 USDC (recommended for successful withdrawals)</p>
                </div>
